...
- `src/personal_services/store.kotoba` — `Store` protocol +
  `MemStore`: registered clients, committed records, an append-only
  audit ledger.
- `src/personal_services/advisor.kotoba` — `Advisor` protocol;
...
- `src/personal_services/governor.kotoba` —
...
- `src/personal_services/actor.kotoba` — `build-graph`, `run-request!`,
