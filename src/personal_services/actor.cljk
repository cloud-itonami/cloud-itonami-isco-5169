(ns personal-services.actor
  "PersonalServicesActor — the ISCO-08 5169 independent
  personal-services actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  service/log operation request (intake → advise → govern → decide →
  ...
  The unconditional invariant: the PersonalServicesAdvisor can never
  directly commit a record or dispatch a robot action the
  PersonalServicesGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [personal-services.advisor :as advisor]
            [personal-services.governor :as governor]
            [personal-services.store :as store]))

(defn build-graph
  "Build a compiled PersonalServicesActor graph. `store` implements
  `personal-services.store/Store`. `advisor` implements
  `personal-services.advisor/Advisor` (defaults to `mock-advisor`).
