(ns personal-services.advisor
  "PersonalServicesAdvisor — proposes a personal-services operation
  (service step, log entry, direct physical contact service,
  handling a client health disclosure) for a registered client. The
  advisor is swappable: `mock-advisor` (deterministic, default in
  dev/tests/CI) or `llm-advisor` (wraps a real
  `langchain.model/ChatModel`). Either way the advisor ONLY produces a
  PROPOSAL — it never writes to the store and has no notion of client
  provenance or contact/health-disclosure risk;
  `personal-services.governor` is the independent system that decides
  whether the proposal may proceed, per the itonami actor pattern.

  A proposal is a map:
    {:op :service|:log|:direct-physical-contact-service|:handle-health-disclosure
...
(def ^:private system-prompt
  "You are a personal-services advisor. Given a service or log
